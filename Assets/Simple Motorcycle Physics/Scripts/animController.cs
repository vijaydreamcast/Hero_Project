...
public class animController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude < 1f)
        {
            anim.CrossFade("Idle",0.3f);
        }
        
        if (transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude > 1f && transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude < 5f)
        {
            anim.CrossFade("acc",0.3f);
            
        }
        
        if (transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude > 5f && transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude < 10f)
        {
        ...
        {
            if(transform.root.GetComponent<Rigidbody>().linearVelocity.magnitude > 10f)
