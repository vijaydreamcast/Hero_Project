...
namespace SBPScripts
{
    public class CyclistAnimController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                if (Input.GetKeyDown(KeyCode.Return) && bicycleController.transform.InverseTransformDirection(bicycleController.rb.linearVelocity).z <= 0.1f && waitTime == 0)
                {
                    waitTime = 1.5f;
            ...
            speed = bicycleController.transform.InverseTransformDirection(bicycleController.rb.linearVelocity).z;
