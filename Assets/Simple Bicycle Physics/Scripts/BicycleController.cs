...
namespace SBPScripts
{
    ...
    public class BicycleController : MonoBehaviour
    {
        ...
        [SerializeField] ArduinoInitilaze arduino;
        Queue<float> steerBuffer = new Queue<float>();
        int bufferSize = 5; // adjust for more smoothing
        [SerializeField] bool IsArduinoEnable = false;
        ...
        void ApplyArduinoInput()
        {
            float normalizeSteer = Mathf.InverseLerp(-22f, 26f, arduino.arduinoAngle);
            customSteerAxis = (SmoothSteer(normalizeSteer) - 0.5f) * 2f;

            customAccelerationAxis = Mathf.InverseLerp(400f, 1023f, arduino.throttle);
            rawCustomAccelerationAxis = customAccelerationAxis;
            customLeanAxis = customSteerAxis;

            //sprint = (arduino.throttle > 900f);
            /*if (arduino.leftBrake == 1 && arduino.rightBrake == 1)
            {
                rb.linearVelocity = Vector3.Lerp(rb.linearVelocity, Vector3.zero, Time.deltaTime * 4f);
            }
            if (arduino.leftBrake == 1 || arduino.rightBrake == 1)
            {
            ...
            if (arduino.leftBrake == 1 || arduino.rightBrake == 1)
            {
                float brakeStrength = (arduino.leftBrake == 1 && arduino.rightBrake == 1) ? 9f : 4f; // full vs half brake
                ApplyBrake(brakeStrength);
            }

            if (Input.GetKeyDown(KeyCode.R))
            {
                arduino.SendMessage("R");
            }
        }
        float SmoothSteer(float newValue)
        {
            steerBuffer.Enqueue(newValue);
            if (steerBuffer.Count > bufferSize)
                steerBuffer.Dequeue();

            float sum = 0f;
            foreach (var v in steerBuffer)
                sum += v;

            return sum / steerBuffer.Count;
        }
        void ApplyBrake(float brakeStrength)
        {
            Vector3 vel = rb.linearVelocity;
            Vector3 brakeForce = -vel.normalized * brakeStrength;
